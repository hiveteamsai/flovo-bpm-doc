...
| **ProcessExecutionState** (motor yürütme durumu) | `process-instance.md` · `workflow-projection.md` (`executionState`) | `new` · `running` · `waiting` · `failed` · `done` · `cancelled` (📝 v0.44 öneri) | [`process-execution-state.md`](./process-execution-state.md) |
| **WorkflowEventType** (yürütme olayı) 📝 v0.44 | `processInstances/workflow-event.md` (`eventType`) | `startRequested` · `stepStarted` · `stepCompleted` · `stepSkipped` · `suspended` · `actionTaken` · `timerFired` · `stepFailed` · `failed` · `recovered` · `cancelled` · `ended` | [`workflow-event-type.md`](./workflow-event-type.md) |
| **WorkflowWaitReason** (bekleme sebebi) 📝 v0.44 | `processInstances/workflow-projection.md` (`waitReason`) | `humanTask` · `processing` · `timer` · `retry` · `subProcess` | [`workflow-wait-reason.md`](./workflow-wait-reason.md) |
| **WorkflowTimerKind** (zamanlayıcı türü) 📝 v0.44 | `processInstances/workflow-timer.md` (`kind`) | `stepTimer` · `taskTimeout` · `retry` · `serviceTriggerCron` | [`workflow-timer-kind.md`](./workflow-timer-kind.md) |
| **WorkflowTimerStatus** 📝 v0.44 | `processInstances/workflow-timer.md` (`status`) | `armed` · `fired` · `deferred` · `cancelled` | [`workflow-timer-status.md`](./workflow-timer-status.md) |
| **WorkflowErrorClass** (hata sınıfı) 📝 v0.44 | `workflow-event.md` (`stepFailed.payload.errorClass`) · `workflow-projection.md` (`lastError`) | `transient` · `permanent` · `design` · `inDoubt` · `guard` | [`workflow-error-class.md`](./workflow-error-class.md) |
...
| **WidthMode** (genişlik modu) | `view-profile-property.md` (`widthMode` / `mobileWidthMode`) | `fraction` (kesir, vars.) · `fill` (doldur) | [`width-mode.md`](./width-mode.md) |
