# Model — FormAwaitingUser (formu bekleyen kullanıcı/grup)
...
> aksiyon alabilecek kullanıcılar **doğrudan bu tablodan** tespit edilir — `ProcessStepExecution` tablosunu filtreleyerek
...
| `formId` | int | FK → Form.id | Beklenen form. |
| `processStepExecutionId` | int | FK → ProcessStepExecution.id | Bekleme hangi adım çalıştırmasında oluştu. |
...
Adımın atama tipine göre **bir veya birden çok** `FormAwaitingUser` kaydı oluşur:
...
Workflow ilerlerken bir **Kullanıcı / Kullanıcı Grubu / Processing** adımına gelindiğinde, o form üzerinde aksiyon
...
## İlişkiler
- **N – 1** → `Form` (`formId`), `ProcessStepExecution` (`processStepExecutionId`),
  `User` (`userId`), `UserGroup` (`userGroupId`).
- **1 – N** ← `UserGroupApprovedUser.formAwaitingUserId` (grup bekleme durumunda onaylayanlar).
