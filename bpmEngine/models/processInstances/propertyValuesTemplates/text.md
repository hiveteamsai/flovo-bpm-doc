...
**Değer YOK — girdi değildir.** `text`, başlık/açıklama gösteren **statik label**'dır; içeriği `defaultValue` (tasarım-zamanı, `Property`'de) + tipografi ayarlarıdır (`fontSize`/`isBold`/`textAlignment`…). Kullanıcı bir değer üretmez → `data`'da **anahtar bulunmaz**.
