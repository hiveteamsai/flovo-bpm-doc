...
> Bu adıma, önceki süreçten bir aksiyonla `parameters: { instanceId }` gelir.

## Diyagram
```mermaid
flowchart LR
  prev[(önceki süreç · instanceId)] --> pdf[createPdf · PDF Oluştur · HTTP Request]
...
**Bu adıma gelen parametre:** `parameters: { instanceId }` (önceki süreçten).
**Ayarlar ve çalışma:**
- `endpoint`: müşteri sunucusunun PDF üretim ucu · `method`: `POST`
- `body`: `{ instanceId }` (gelen parametreden)
...
> İlgili tasarım: `../../service-settings/process-step.md` (HTTP Request) · `../../architectures/api/flovo-customer-api.md`.
