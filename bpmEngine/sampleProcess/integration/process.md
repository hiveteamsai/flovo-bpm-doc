...
> **Not:** Boş bırakılan adıma kadar bir akış vardır; bu bölüm **entegrasyon kısmıdır**. Adıma `parameters: { formId }` gelir.

## Diyagram
```mermaid
flowchart LR
  prev[(önceki süreç · formId)] --> start[integrationStart · Entegrasyon Başlat · Kullanıcı]
...
**Bu adıma gelen parametre:** `parameters: { formId }` (önceki süreçten veya `transferFail` ile geri dönüşte
`{ formId, errorMessage }`).
...
**Aksiyonlar:**
- **`startTransfer` (Manuel):** Kullanıcı tetikler. Hedef adım `integrationAsync`. Taşıdığı veri: `parameters: { formId }`.
...
**Ayarlar ve çalışma:**
- `endpoint`: müşteri sunucusu entegrasyon ucu · `method`: `POST` · `body`: `{ formId }`
...
**Aksiyonlar:**
- **`default` (otomatik, async):** Hedef adım `transferWaiting`. Taşıdığı veri: `parameters: { formId }`.
...
- **`transferOk` (Webhook):** Müşteri sunucusu **başarıyla** bitince Flovo Customer API ile çağrılır. Hedef adım
  `processEnd`. Taşıdığı veri: `parameters: { formId }` (+ varsa sonuç).
- **`transferFail` (Webhook):** Aktarım **başarısız** olursa çağrılır. Hedef adım `integrationStart` (başa dön).
  Taşıdığı veri: `parameters: { formId, errorMessage }`.
