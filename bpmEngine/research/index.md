...
| **engine-runtime/** | **Motor runtime anlatımı** — v0.44 çalışma-zamanı mimarisi taslak setinin (WorkflowEvent/Projection/Timer · iki-TX adım · idempotency · hata · zamanlayıcı · saklama/KVKK · R1–R17 · Q1–Q22) tek sayfalık **özet + detay HTML** anlatımı + **metot-düzeyi mimari şeması** (çağrı ağacı · akış diyagramları · metot × tablo matrisi; R18/Q23 önerili). 📝 **Anlatım/sunum çıktısı** — bağlayıcı metin `engine-runtime*.md` + model dokümanlarıdır; set onay bekliyor. | [`engine-runtime/index.md`](./engine-runtime/index.md) |
