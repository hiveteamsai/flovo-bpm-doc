...
- **`relatedPropertyIds`** — kaldırıldı (v0.45): eski uygulamada **DataGrid'in alt-alan (kolon) listesi**ydi — `PropertyDto.relatedPropertyIds` (id string) +
  `relatedPropertyDtos` (alt `PropertyDto` listesi) çifti; yalnız `DataGridPropertySettings` sayfasında düzenlenir, kontrol tipi değişince temizlenir; iş kuralları
  grid alt-alanına `childField`/`dataGridPropertyDtos` ile ulaşır (→ `../current-flovo-bpm-engine/work-rules-detail.md` §6.4). **DataGrid yeni tasarımda
  kaldırıldığı** (yukarıda) ve alt-alanlı tipler (`keyValueList`, `groupByTaxReceipt`) alt-alanlarını **sabit değer şablonuyla** tanımladığı için dinamik
  alt-alan listesine ihtiyaç kalmadı; `parentProperty` ile ilgisi yoktu. Parent Property ilişkisi `parentPropertyId` (**bağlayan alan**) + `refPropertyId` (**üst servisteki yansıtılan alan**) ile tanımlı.
